use std::{env, fs, process, sync::mpsc};
...
fn main() {
    if let Err(error) = pollster::block_on(run()) {
        eprintln!("comparative gpu receipt failed: {error}");
        process::exit(1);
    }
}
...
async fn run() -> Result<(), String> {
    ...
    if args.len() != 2 {
        return Err(
            "usage: m11_comparative_gpu_receipt <before.json> <after.json>".into(),
        );
    }
    let left_raw = fs::read_to_string(&args[0])
    ...
    let shared = specimen
        ...
        .ok_or_else(|| {
            "comparative pair has no shared visible node with source/provenance closure".to_string()
        })?;
    ...
    let before_plan = prepare_graph_gpu_plan(&specimen.read_model.topology.before)?;
    let after_plan = prepare_graph_gpu_plan(&specimen.read_model.topology.after)?;
    let delta_plan = prepare_graph_gpu_plan(&specimen.read_model.topology.delta)?;
    ...
    if before_state != after_state || before_state != shell_state {
        return Err(format!(
            "comparative pick parity failed: before={before_state:?}, after={after_state:?}, shell={shell_state:?}"
        ));
    }
...
fn pick_object(
    ...
    let gpu_input = decode_graph_pick(Some(pick_id), plan);
    Ok(reduce_selection(
        decode_gpu(gpu_input),
        SelectionState::default(),
    ))
}
